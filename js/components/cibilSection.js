// Check CIBIL Component (Official Brand Score Check & Festive Offer Unlock)
...
import { getUserRewards, isActivityClaimed, saveRewardClaim } from '../state/rewardState.js';
...
export function renderCibilSection(container, onNavigate) {
  ...
  const baseCibilUrl = 'https://poonawallafincorp.com/credit-score/check-cibil-score?uat=festive_campaign_2025';
  const finalCibilUrl = buildUtmUrl(baseCibilUrl, 'cibil-check');
  const isCibilClaimed = isActivityClaimed('check_cibil');

  wrapper.innerHTML = `
    <div class="section-header align-left">
      <span class="section-kicker" style="color: var(--wf-text-secondary); font-size: 0.75rem; font-weight: 700; letter-spacing: 0.05em; text-transform: uppercase;">• OFFICIAL CREDIT HEALTH & REWARDS</span>
      ...
      <p class="subheading">Check your credit score for free, auto-generate your pre-approved festive loan rate, and get your voucher via WhatsApp.</p>
    </div>
    ...
    <div style="max-width: 800px; margin: 0 auto 12px auto; display: flex; justify-content: flex-start;">
      <span class="prominent-free-badge" style="background: #10B981; color: #FFFFFF; font-weight: 800; font-size: 0.85rem; letter-spacing: 0.08em; padding: 6px 16px; border-radius: 4px; display: inline-flex; align-items: center; gap: 6px; box-shadow: 0 2px 8px rgba(16, 185, 129, 0.25);">
        ✨ 100% FREE OFFICIAL CHECK + GUARANTEED VOUCHER
      </span>
    </div>
    ...
    <div class="cibil-main-card" style="background: var(--wf-surface); padding: 40px 32px; border-radius: var(--radius-lg); border: 1px solid var(--wf-border); box-shadow: var(--shadow-sm); max-width: 800px; margin: 0 auto 48px auto; text-align: center;">
      ...
      <p style="font-size: 0.95rem; color: var(--wf-text-secondary); max-width: 580px; margin: 0 auto 28px auto; line-height: 1.5;">
        Get an instant credit score analysis with complete report breakdown and auto-generate an exclusive festive voucher shared directly to your WhatsApp!
      </p>
      ...
      <div style="background: var(--wf-surface-subtle); border: 1px solid var(--wf-border); border-radius: var(--radius-md); padding: 20px 24px; margin-bottom: 28px; text-align: left;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
          ...
          <span style="font-size: 0.82rem; font-weight: 700; color: #10B981;">750+ Unlocks Best Festive Rates</span>
        </div>
      ...
      <div style="display: flex; flex-direction: column; gap: 14px; align-items: center;">
        <button class="btn-primary glow-effect" id="cibil-check-and-unlock-btn" style="padding: 16px 48px; font-size: 1.1rem; border-radius: var(--radius-sm); display: inline-flex; align-items: center; gap: 10px; background: #10B981; border-color: #10B981; color: #FFFFFF; font-weight: 700;">
          🔍 Check Score & Auto-Generate Offer
        </button>

        <a href="${finalCibilUrl}" target="_blank" rel="noopener noreferrer" class="btn-secondary" id="check-cibil-cta" style="padding: 12px 32px; font-size: 0.92rem; border-radius: var(--radius-sm); display: inline-flex; align-items: center; gap: 8px; text-decoration: none; background: #FFFFFF; border-color: var(--wf-border); color: var(--wf-text-primary);">
          Official Full CIBIL Portal ↗
        </a>
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 16px; margin-top: 36px; padding-top: 28px; border-top: 1px solid var(--wf-border); text-align: left;">
        ...
        <div style="display: flex; align-items: flex-start; gap: 10px;">
          <span style="color: #10B981; font-weight: bold; font-size: 1.1rem;">✓</span>
          <div>
            <strong style="font-size: 0.85rem; display: block; color: var(--wf-text-primary);">Auto Offer on WhatsApp</strong>
            <span style="font-size: 0.78rem; color: var(--wf-text-secondary);">Instant voucher code dispatched to WhatsApp</span>
          </div>
        </div>
        <div style="display: flex; align-items: flex-start; gap: 10px;">
          <span style="color: #10B981; font-weight: bold; font-size: 1.1rem;">✓</span>
          <div>
            <strong style="font-size: 0.85rem; display: block; color: var(--wf-text-primary);">Pre-Approved Rates</strong>
            <span style="font-size: 0.78rem; color: var(--wf-text-secondary);">Unlock best festive rates starting 9.99% p.a.</span>
          </div>
        </div>
      </div>
    </div>
  ...
  renderBreadcrumbs(wrapper, [{ label: 'Check CIBIL', route: 'cibil' }], onNavigate);

  // Check Score & Auto-Generate Offer Button Handler
  const checkAndUnlockBtn = wrapper.querySelector('#cibil-check-and-unlock-btn');
  if (checkAndUnlockBtn) {
    checkAndUnlockBtn.addEventListener('click', () => {
      requireAuth(() => {
        const userRewards = getUserRewards();
        const allocated = allocateRewardForGame('check_cibil', userRewards.claims);
        saveRewardClaim('check_cibil', allocated.deal.dealId);
        
        const session = getSession();
        trackGa4Event(GA4_EVENTS.CIBIL_SCORE_CHECKED, { score: 785 });
        sendLeadToLeadSquared({
          mobileNumber: session.mobile,
          activityType: 'CIBIL Checked & Offer Generated',
          rewardAllocated: allocated.deal.dealId,
          contentSlug: 'cibil-check'
        });

        showWhatsAppNotification({
          title: 'CIBIL Offer Generated!',
          recipient: session.mobile,
          message: `Your festive pre-approved loan offer with guaranteed ${allocated.deal.brandName} voucher is dispatched to your WhatsApp!`,
          voucherCode: allocated.deal.couponCode,
          brand: allocated.deal.brandName,
          actionUrl: finalCibilUrl
        });

        openRewardModal(allocated.deal, {
          activityKey: 'check_cibil',
          onNavigate
        });
      });
    });
